...
function NationSocialMediaEdit(){

    const [name, setName] = useState('');
    const [url, setUrl] = useState('');
    const { team_id, media_id } = useParams();
    let history = useHistory();
    const token = localStorage.getItem('token')

    useEffect(()=> {
        api.get(`api/admin/social_media/${media_id}/edit`, {
            headers : {
    ...
    async function onSubmit(e){
        ...
        const response = await api.post(`api/admin/social_media/${media_id}/update`, data, {
            headers : {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'Authorization': 'Bearer ' + token
...
export default NationSocialMediaEdit;