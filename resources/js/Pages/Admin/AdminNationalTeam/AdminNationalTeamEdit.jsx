...
function NationalTeamEdit(){
    ...
    useEffect(()=> {
        api.get(`/api/admin/national-team/${id}/edit`, {
            headers : {
            ...
            if(response.data.status === 'success'){
                ...
                setWAffiliation(response.data[0].world_federation_afilliation)
                setWorldFederation(response.data[0].world_federation)
                setWDate(response.data[0].world_affiliation_date)
                setCAffiliation(response.data[0].continental_federation_afilliation)
                setContinentalFederation(response.data[0].continental_federation)
                setCDate(response.data[0].continental_affiliation_date)
    ...
    async function onSubmit(e){
        ...
        var data = JSON.stringify({
            ...
            world_federation_afilliation : wAffiliation,
            world_federation : worldFederation,
            world_affiliation_date : wAffilDate,
            continental_federation_afilliation : cAffiliation,
        ...
        const response = await api.post(`/api/admin/national-team/${id}/update`, data, {
            headers : {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'Authorization': 'Bearer ' + token
    ...
    return (
        <>
            <HeaderAdmin titulo={'Editar Seleção'} />
                                ...
                                <div className="card-body">
                                    <form onSubmit={onSubmit}>
                                        ...
                                        <div className="form-group row">
                                                ...
                                                <div className="col-12 col-xs-12 col-md-12">
                                                    <div className="form-check form-check-inline">
                                                        <input className="form-check-input" type="radio" name="wAffiliation" id="yes" value={wAffiliation} checked={wAffiliation == '1' ? 'checked' : ''} onClick={() =>setWAffiliation('1')} required/>
                                                        <label className="form-check-label" htmlFor="yes">Sim</label>
                                                    </div>
                                                    <div className="form-check form-check-inline">
                                                        <input className="form-check-input" type="radio" name="wAffiliation" id="no" value={wAffiliation} checked={wAffiliation == '0' ? 'checked' : ''} onClick={() =>setWAffiliation('0')}/>
                                        ...
                                        <div className="form-group row">
                                                ...
                                                <div className="col-12 col-xs-12 col-md-12">
                                                    <div className="form-check form-check-inline">
                                                        <input className="form-check-input" type="radio" name="cAffiliation" id="yes" value={cAffiliation} checked={cAffiliation == '1' ? 'checked' : ''} onClick={() =>setCAffiliation('1')} required/>
                                                        <label className="form-check-label" htmlFor="yes">Sim</label>
                                                    </div>
                                                    <div className="form-check form-check-inline">
                                                        <input className="form-check-input" type="radio" name="cAffiliation" id="no" value={cAffiliation} checked={cAffiliation == '0' ? 'checked' : ''} onClick={() =>setCAffiliation('0')}/>
                                                        <label className="form-check-label" htmlFor="no">Não</label>
                                                    </div>   
                                                </div>
                                            </div>
